...
if (text === '\u{21a9} назад') {
    // Определяем, куда нужно вернуться из текущего блока
    const backMessages = {
    ...
    // Определяем текущую позицию и куда нужно вернуться
    if (user.previous) {
        user.current = user.previous; // Возвращаем в предыдущий блок
        user.previous = null; // Сбрасываем переменную предыдущего блока
    }
