...
vk.updates.on('message_new', async (context) => {
    ...
    if (text === '\u{21a9} назад') {
        // Логика для кнопки "Назад" из блоков info, dates, payment, booking
        if (user.current === 'info' || user.current === 'dates' || user.current === 'payment' || user.current === 'booking') {
            user.current = 'main';  // Возвращаем в главный блок
        }
    
        // Логика для кнопки "Назад" из блока faq
        if (user.current === 'faq') {
            user.current = 'booking';  // Возвращаем в блок бронирования
        }
    ...
    return context.send({
        message: backMessages[user.current] || "Главное меню. Выберите подходящий вариант!",
