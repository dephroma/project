...
vk.updates.on('message_new', async (context) => {
    ...
    if (message.includes('как дела')) 
        { await context.send('У меня всё отл123ично,asdasd сфывфвaadыпаasфвафывфвфваdasdсибо! А как у вас?');
            await sendMainMenu(context);
        } 
    if (message === 'привет' || message ==='здравствуйте' || message ==='начать' || message ==='приветствую' || message ==='салам' || message ==='салам алейкум') {
        await context.send('Здравствуйте! Покафыв наш менеджер спешить отasdветить, я смогу помочь вам с поиском нужной информации. Напишите "туры", "даты", "цены/оплата" или "информация" для получения данных.');}
        else if (message === 'туры') 
            { await context.send('Доступные к приоб1ретению туры вы можете посмотреть здесь. Выберите тур и нажмите "связаться с нами" для получения полной программы тура и дополнительной информации'); }
