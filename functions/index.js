...
vk.updates.on('message_new', async (context) => {
    const message = he.decode(context.text.trim()).toLowerCase(); // Декодируем текст сообщения
            ...
            keyboard: Keyboard.keyboard([
                [
                    Keyboard.textButton({
                        ...
                        payload: JSON.stringify({ button: 'catalog' }) // Переход по кнопке "Каталог и бронирование"
                    })
                ],
                [
                    Keyboard.textButton({
...
vk.updates.on('message_new', async (context) => {
    ...
    try {
        ...
        console.log('Payload:', payload); // Логируем payload для отладки
    } catch (e) {
